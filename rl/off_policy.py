...
class OffPolicyRLHandler(RLHandler):
    ...
    def dump_logs(self) -> None:
        ...
        self.model.logger.record("time/episodes", self.model._episode_num, exclude="tensorboard")
        try:
        ...
        if self.model.use_sde:
            self.model.logger.record("train/std", (self.model.actor.get_std()).mean().item())

        if len(self.model.ep_success_buffer) > 0:
            self.model.logger.record("rollout/success_rate", safe_mean(self.model.ep_success_buffer))
